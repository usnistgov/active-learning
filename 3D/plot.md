...
```python
def plot_scores(scores, opt=None, opt_error=None, error_freq=20, scoring='mse', ylog=False, scale=1.0, fontsize=20):
...
```python
plt, ax = plot_scores(output, error_freq=100, opt=opt, opt_error=err, scoring=scoring, ylog=ylog, scale=scale, fontsize=22)
plt.title('(a)')
